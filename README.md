...
3. **Local market state (5m only)** – Every trade call pulls the latest 5-minute bars from Supabase and computes a deterministic market state using an EMA21 slope normalized by ATR. The output is tiny (`trend`, `confidence`, `slope_norm`, `signal`) and is the single source of truth for trend bias.【F:api.py†L929-L957】【F:market_regime.py†L41-L90】
4. **Risk guardrails** – `PositionManager` calculates account health and blocks trades when can_trade is false (daily loss/target or consecutive losses). Sideways/low-confidence states also force an immediate HOLD with no orders sent.【F:tradingview_projectx_bot.py†L686-L714】【F:position_manager.py†L187-L231】
5. **Optional AI adjudication** – AI-enabled accounts send a compact payload (alert, symbol, 5m market_state, and risk/position context) to the configured webhook. The AI may nudge sizing or signal, but execution remains simple market BUY/SELL/HOLD with server-side brackets handled by Topstep.【F:api.py†L900-L924】【F:tradingview_projectx_bot.py†L714-L742】
6. **Strategy execution** – Only the `run_simple` strategy remains; it sends a market order, flattens opposing exposure, and logs to SignalR. Protective brackets are broker-managed server-side.【F:tradingview_projectx_bot.py†L837-L872】【F:strategies.py†L24-L67】

The result is a minimal, deterministic loop where TradingView alerts are filtered by local market state and risk checks before a single market order path touches ProjectX.
...
- **Market Context**: `market_regime.py` now implements a lightweight EMA21 slope check built from aggregated 1m OHLC data. `api.get_market_conditions_summary` exposes the resulting trend snapshot for logs, endpoints, and strategy adjustments.
