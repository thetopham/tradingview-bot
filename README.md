...
- **Alert Handling**: `/webhook` accepts TradingView payloads, validates `WEBHOOK_SECRET`, and hands them to `handle_webhook_logic` on a background thread. The handler resolves accounts/contracts via `config.py` + `api.get_contract`, blocks trading during the get-flat window, and feeds a single bracket executor that consumes high-level AI actions (enter/exit/add/reduce).
- **Strategy Layer**: `strategies.execute_bracket_decision` submits one server-side bracket order using the configured template—no client-managed SL/TP or polling loops. Each trade session is tracked through `signalr_listener.track_trade`, which later logs outcomes to Supabase.
- **External Services**:
  ...
  - n8n AI endpoints (mapped in `.env`) receive OHLC arrays (not screenshots) for multi-timeframe reasoning and can veto trades for AI-managed accounts (`epsilon`, `beta` by convention).
...
- **Market Context**: `market_regime.py`, `market_regime_ohlc.py`, and `market_regime_hybrid.py` now rely on tv_datafeed OHLC aggregation across multiple timeframes (5m–1D) to derive regimes. `api.get_market_conditions_summary` exposes a normalized snapshot for logs, endpoints, and strategy adjustments.
