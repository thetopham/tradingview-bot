...
def place_bracket_order(acct_id: int, cid: str, side: int, size: int, template: str,
                        time_in_force: Optional[str] = None, custom_tag: Optional[str] = None):
    """
    Submit a single server-side bracket order using a pre-defined template.
    This replaces multi-call stop/limit management and avoids follow-up polling.
    """
    payload = {
        "accountId": acct_id,
        "contractId": cid,
        "type": 8,  # Assume bracket template order type supported by Topstep
        "side": side,
        "size": size,
...
def fetch_multi_timeframe_analysis(n8n_base_url: str, timeframes: List[str] = None,
    ...
    """
    Fetch multi-timeframe analysis using OHLC-only data to avoid screenshot and n8n image calls.
    """
    import datetime
    ...
    if timeframes is None:
        timeframes = config.get('OHLC_TIMEFRAMES', ['5m', '15m', '30m', '1h', '4h', '1d'])
    ...
    try:
        ...
        try:
            ...
            if result.data and len(result.data) >= 50:
                ...
                for tf_name in timeframes:
                    tf_minutes = timeframe_mapping.get(tf_name)
                    if not tf_minutes:
                        logging.debug(f"Skipping unsupported timeframe {tf_name}")
                        continue
                    try:
                        aggregated_bars = aggregate_1m_to_timeframe(minute_bars, tf_minutes)
                        if len(aggregated_bars) >= 5:
                            ohlc_data[tf_name] = {
                            ...
                            logging.info(f"✅ Aggregated {len(aggregated_bars)} bars for {tf_name}")
                        else:
        ...
        if ohlc_data:
            try:
                ...
                logging.info(
                    "🚀 OHLC analysis SUCCESS: %s (confidence: %s%%) via %s",
                    regime_analysis['primary_regime'],
                    regime_analysis['confidence'],
                    regime_analysis.get('analysis_method', 'ohlc_only')
                )
                snapshot = {
                    'ohlc_data_summary': {
                        tf: {
                    ...
                    'data_stats': {
                        'ohlc_timeframes': list(ohlc_data.keys()),
                ...
                logging.error(f"OHLC-only analysis failed: {e}", exc_info=True)

        logging.error("No OHLC data available for regime analysis")
        fallback = {
            'ohlc_data_summary': {},
            ...
            'data_sources': [],
            'data_stats': {'ohlc_timeframes': [], 'total_sources': 0}
        }
        return fallback
    except Exception as e:
...
def ai_trade_decision_with_regime(account, strat, sig, sym, size, alert, ai_url):
    ...
    try:
        ...
        payload = {
            ...
            "timeframe_signals": {tf: 'UNKNOWN' for tf in ohlc_summary.keys()},
