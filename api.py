...
from market_regime import MarketStateAnalyzer
...
def get_market_conditions_summary(force_refresh: bool = False) -> Dict:
    """Compute market conditions locally from 5m Supabase bars."""
    try:
        ...
        bars = list(reversed(result.data)) if result.data else []
        market_state = market_state_analyzer.compute_market_state(bars)

        summary = {
            'timestamp': market_state.get('timestamp') or datetime.now(CT).isoformat(),
            'regime': market_state.get('regime', 'sideways'),
            'confidence': market_state.get('confidence', 0),
            'trade_recommended': market_state.get('trade_recommendation', False),
            'risk_level': market_state.get('risk_level', 'high'),
            'key_factors': market_state.get('supporting_factors', []),
        ...
        return {
            ...
            'volatility': 'unknown',
            'signal': 'HOLD',
            'market_state': market_state_analyzer._fallback('unavailable'),
        }
...
def ai_trade_decision_compact(account, strat, sig, sym, size, alert, ai_url, market_state: Dict,
                              position_context: Dict = None, risk_status: Dict = None):
    """Lightweight AI request that only ships the local market state and risk context."""
    payload = {
        "account": account,
        "strategy": strat,
        "signal": sig,
    ...
    try:
        ...
        logging.error(f"AI error (compact): {exc}")
        return {
            "strategy": strat,
            "signal": "HOLD",
            "account": account,
