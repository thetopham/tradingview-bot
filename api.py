...
def place_bracket_order(acct_id: int, cid: str, side: int, size: int, bracket_template: str):
    """
    Submit a single server-side bracket order using a named template configured in Topstep.
    """
    logging.info(
        "Placing bracket order acct=%s cid=%s side=%s size=%s template=%s",
        acct_id, cid, side, size, bracket_template
    )
    payload = {
        "accountId": acct_id,
        "contractId": cid,
        "side": side,
        "size": size,
        "bracketTemplate": bracket_template,
    }
...
def fetch_multi_timeframe_analysis(n8n_base_url: str, timeframes: List[str] = None,
    ...
    """
    Build an OHLC-only multi-timeframe snapshot for the AI and regime detectors.

    The `n8n_base_url` parameter is retained for backward compatibility but is no
    longer used; analysis is derived solely from Supabase tv_datafeed OHLC bars.
    """
    import datetime
    ...
    if timeframes is None:
        timeframes = config.get('AI_TIMEFRAMES', ['5m', '15m', '30m'])
    ...
    try:
        logging.info("Starting OHLC-only regime analysis")
        ...
        def tf_to_minutes(tf_label: str) -> int | None:
        ...
        try:
            ...
            if result.data and len(result.data) >= 50:
                ...
                for tf_name in timeframes:
                    ...
                    if not minutes:
                        logging.warning(f"Skipping unknown timeframe label: {tf_name}")
                        continue
                    try:
        ...
        if not ohlc_data:
            logging.error("No OHLC data available; returning fallback regime")
            return {
                'ohlc_data': {},
        ...
        try:
            logging.info(f"🎯 Running OHLC regime detection with timeframes: {', '.join(ohlc_data.keys())}")
            ...
            logging.info(f"🚀 OHLC analysis SUCCESS: {regime_analysis['primary_regime']} "
            ...
            snapshot = {
                ...
                'ohlc_data_summary': {
                    tf: {
                        ...
                        'latest_rsi': data['rsi'][-1] if data.get('rsi') else None
                    }
            ...
            logging.error(f"OHLC regime analysis failed: {e}", exc_info=True)
            return {
                'ohlc_data': ohlc_data,
                'regime_analysis': market_regime_analyzer._get_fallback_regime(f"Analysis failed: {e}"),
                'timestamp': now.isoformat(),
                'analysis_method': 'ohlc_only',
                'data_sources': data_sources,
            }
...
def evaluate_entry_quality(regime_analysis: Dict, position_context: Dict = None) -> Dict:
    ...
    if cross_val:
        if cross_val.get('agreement', True):
            quality_score += 20
            factors.append("Signals agree across sources")
        else:
            quality_score -= 10
            factors.append("Signal disagreement across sources")
...
def ai_trade_decision_with_regime(account, strat, sig, sym, size, alert, ai_url):
    """
    Enhanced AI trade decision that includes market regime analysis, OHLC payloads, and position context
    """
    try:
        market_analysis = fetch_multi_timeframe_analysis("", force_refresh=False)
        regime = market_analysis['regime_analysis']
        regime_rules = market_regime_analyzer.get_regime_trading_rules(regime['primary_regime'])
        timeframe_analysis = regime.get('timeframe_analysis', {})
        default_bracket = config.get('ACCOUNT_BRACKETS', {}).get(
            account.lower(),
            config.get('BRACKET_TEMPLATE', 'default')
        )
        ...
        payload = {
            ...
            "ohlc_summary": market_analysis.get('ohlc_data_summary', {}),
            "market_analysis": {
                "regime": regime['primary_regime'],
        ...
        if 'direction' not in data:
            data['direction'] = data.get('signal', sig)
        if 'bracket' not in data:
            data['bracket'] = default_bracket
        ...
        if data.get('action') == 'enter' and data.get('direction') in ('BUY', 'SELL'):
            data['action'] = 'enter'
        elif data.get('action') == 'exit':
            data['direction'] = 'FLAT'
        elif data.get('action') not in ('enter', 'exit', 'add', 'reduce', 'hold'):
            data['action'] = 'hold'
        return data
    except Exception as e:
...
def get_market_conditions_summary(force_refresh: bool = False) -> Dict:
    ...
    """
    try:
        market_analysis = fetch_multi_timeframe_analysis("", force_refresh=force_refresh)
