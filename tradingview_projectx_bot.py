...
def in_get_flat_zone(now=None):
    """Return True if now (datetime or dtime) is between GET_FLAT_START and GET_FLAT_END CT."""
    if now is None:
        now = datetime.now(CT)
...
def tv_webhook():
    ...
    # always flatten on FLAT or during the 3:10–5:00 PM CT window
    ...
    # if opposite exists, flatten it first
    ...
    # 2) Fill price via recent trades
    trades = [
        t for t in search_trades(datetime.utcnow() - timedelta(minutes=5))
        if t["orderId"] == oid
    ]
