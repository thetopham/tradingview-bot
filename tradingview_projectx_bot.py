...
def handle_webhook_logic(data):
    try:
        ...
        symbol = data.get("symbol", MARKET_SYMBOL)
        ...
        if sig == "FLAT":
            try:
                ...
                logging.info("Manual FLAT executed for account %s symbol %s", acct, symbol)
            except Exception as exc:
                logging.error("Failed FLAT for %s/%s: %s", acct, symbol, exc)
            return

        supabase = get_supabase_client()
        market_state = build_market_state(supabase, symbol=symbol)
        ...
        slopes = market_state.get("slope") if isinstance(market_state, dict) else {}
        logging.info(
            "Evaluation account=%s regime=%s slopes=%s action=%s reason=%s trading_enabled=%s",
            acct,
        ...
        result = send_entry(plan.get("action"), acct_id, symbol, size, TRADING_ENABLED)
        if not TRADING_ENABLED:
            logging.info(
                "SIMULATED ORDER account=%s action=%s size=%s result=%s",
                acct,
                plan.get("action"),
                size,
                result,
            ...
            logging.info("Order dispatch result: %s", result)
