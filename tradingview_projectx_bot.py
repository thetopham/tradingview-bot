...
from api import (
    flatten_contract, get_contract, search_pos, get_supabase_client, search_trades, search_open,
...
from execution import send_entry, TRADING_ENABLED, DEFAULT_TRADE_SIZE
...
def handle_webhook_logic(data):
    try:
        ...
        requested_size = int(data.get("size", DEFAULT_TRADE_SIZE))
        ...
        if in_get_flat(now):
            logging.info("In get-flat window -> HOLD")
            return
        ...
        action_plan = decide(market_state, position_context, now=now)

        logging.info(
            "Market state snapshot tf_slopes=%s regimes=%s errors=%s",
            action_plan.get("details", {}).get("slopes"),
            action_plan.get("details", {}).get("regimes"),
            action_plan.get("details", {}).get("market_errors"),
        )

        if action_plan["action"] in {"BUY", "SELL"}:
            logging.info("Plan decided %s (%s)", action_plan["action"], action_plan["reason_code"])
            send_entry(action_plan["action"], acct_id, symbol, requested_size)
        else:
            logging.info("HOLD decision (%s)", action_plan.get("reason_code"))
