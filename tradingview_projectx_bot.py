...
def handle_webhook_logic(data):
    try:
        ...
        market_state = build_market_state(sym)
        plan = trigger_engine.decide(market_state, position_context)
        logging.info(
            "Action plan for %s: %s (TRADING_ENABLED=%s)",
            acct,
            plan,
            TRADING_ENABLED,
        )
        ...
        if plan.get("action") in ("BUY", "SELL"):
            result = send_entry(plan["action"], acct_id, sym, size=size)
            logging.info("Execution result: %s", result)
        else:
            logging.info("No trade sent (%s)", plan.get("reason_code"))
