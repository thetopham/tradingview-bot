...
from api import (
    flatten_contract, get_contract, get_supabase_client, search_trades,
    search_open, get_market_conditions_summary, search_contracts,
    get_active_contract_for_symbol_cached, get_active_contract_for_symbol,
    CONTRACT_CACHE_DURATION, search_accounts,
)
...
DEFAULT_TRADE_SIZE = config.get('DEFAULT_TRADE_SIZE', 1)
...
def handle_webhook_logic(data):
    try:
        ...
        if acct not in ACCOUNTS:
            logging.error("Unknown account '%s'", acct)
            return
        ...
        try:
            ...
            logging.error("Failed to resolve contract for %s: %s", symbol, exc)
            return
        pm = PositionManager(ACCOUNTS)
        position_context = pm.get_position_context_for_ai(acct_id, contract_id)

        market_state = build_market_state(symbol)
        slopes = {
            tf: state.normalized_slope
            for tf, state in market_state.timeframes.items()
        }
        logging.info("Market state built for %s @ %s slopes=%s", symbol, market_state.as_of, slopes)

        plan = decide(market_state, position_context)
        logging.info("Action plan: %s (%s)", plan.action, plan.reason_code)

        if plan.action in ("BUY", "SELL") and TRADING_ENABLED:
            send_entry(plan.action, acct_id, symbol, size)
        elif plan.action in ("BUY", "SELL"):
            logging.info("TRADING_ENABLED is false; skipping execution for %s", plan.action)
