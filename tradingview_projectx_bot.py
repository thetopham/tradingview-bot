...
# Simple cache for raw‐contract lookups
CONTRACT_CACHE = {}
...
# ─── Contract Lookup ───────────────────────────────────
def _lookup_raw(raw_code: str) -> str:
    """Direct lookup for a full CME contract code."""
    ensure_token()
    resp = requests.post(
        f"{PX_BASE}/api/Contract/search",
        json={"searchText": raw_code, "live": True},
        headers={
            "Content-Type": "application/json",
            "Authorization": f"Bearer {token}"
        }
    ...
    raise ValueError(f"No contract found for code '{raw_code}'")
...
def search_contract(raw_symbol: str) -> str:
    ...
    if contracts:
        return contracts[0]["id"]
    raise ValueError(f"No active contract for symbol '{raw_symbol}'")
...
def tv_webhook():
    ...
    # handle raw CME code vs ticker
    if raw_symbol.startswith("CON."):
        cid = CONTRACT_CACHE.get(raw_symbol) or _lookup_raw(raw_symbol)
        CONTRACT_CACHE[raw_symbol] = cid
    else:
        cid = search_contract(raw_symbol)

    side      = 0 if sig == "BUY" else 1
    exit_side = 1 - side

    # cancel/close only if opposite exists
    ...
    try:
        ...
        if fill_pr is None:
            raise RuntimeError("Could not determine fill price")
