    ...
    try:
        ...
        if sig == "FLAT":
            try:
                ...
                logging.info("FLAT request: flattened account %s contract %s", acct, cid)
            except Exception as exc:
                logging.error("FLAT request failed for %s (%s): %s", acct, sym, exc)
            return

        supabase_client = get_supabase_client()
        market_state = build_market_state(supabase_client, sym)
        cid = get_contract(sym)
        pm = PositionManager(ACCOUNTS)
        ...
        logging.info(
            "Decision: regime=%s slopes=%s action=%s reason=%s trading_enabled=%s account=%s",
        ...
        if action_plan.get('action') in ("BUY", "SELL"):
            send_entry(action_plan['action'], acct_id, sym, size, TRADING_ENABLED)
    except Exception as e:
        import traceback
        logging.error(f"Exception in handle_webhook_logic: {e}{traceback.format_exc()}")
