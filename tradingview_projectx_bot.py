...
from strategies import execute_bracket_decision
...
ACCOUNT_BRACKETS = config.get('ACCOUNT_BRACKETS', {})


def bracket_for_account(account_name: str) -> str:
    return ACCOUNT_BRACKETS.get(account_name.lower(), config.get('BRACKET_TEMPLATE', 'default'))
...
def handle_webhook_logic(data):
    try:
        strat = (data.get("strategy") or "bracket").lower()
        ...
        if acct in AI_ENDPOINTS:
            ...
            decision = {
                ...
                'direction': ai_decision.get('direction', sig),
                'size': ai_decision.get('size', size),
                'bracket': ai_decision.get('bracket') or bracket_for_account(acct),
                'reason': ai_decision.get('reason'),
                'regime': regime,
            }
            logging.info(
                f"AI decision: action={decision['action']} direction={decision['direction']} size={decision['size']} "
                f"bracket={decision['bracket']} regime={regime} ({regime_confidence}%)"
            )
        else:
            ai_decision_id = ai_decision_id or int(time.time() * 1000) % (2**62)
            logging.info(f"Non-AI account {acct} - using direct signal {sig}")
            if sig == "BUY" or sig == "SELL":
                decision = {
                    'action': 'enter',
                    'direction': sig,
                    'size': size,
                    'bracket': bracket_for_account(acct),
                    'regime': 'manual'
                }
            elif sig == "FLAT":
                decision = {
        ...
        if decision.get('action') == 'hold':
            logging.info("Decision result is HOLD; no orders sent.")
            return

        execute_bracket_decision(acct_id, acct, sym, decision, alert, ai_decision_id)
