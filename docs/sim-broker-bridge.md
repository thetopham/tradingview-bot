...
The bridge also renders `api.place_market`, account, position, open-order, trade, cancel, and flatten calls against the local ledger in sim mode. `GET /sim/broker-events` provides cursor-based `GatewayUserAccount`, `GatewayUserOrder`, `GatewayUserPosition`, and `GatewayUserTrade` messages from the same durable ledger events. This is an optional authenticated compatibility endpoint, not a SignalR WebSocket server. The simulator does not poll it to detect fills or closes; those are committed directly to the ledger. Closed trades also enter `/sim/results` for the existing reporting shape, while the publisher reads the durable outbox directly.
