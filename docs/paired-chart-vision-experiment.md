...
At the 2026-09-24 06:10 UTC 5-minute boundary, alpha, beta, and gamma each produced a successful numeric and image decision. Delta did so at 06:15 and 06:30 UTC. Epsilon's first scheduled run at 06:30 UTC completed and logged its chart, but its model reported `imageIncluded=false` because an open editor overwrote its image settings. After closing the editor and republishing, an isolated check and the 07:00 UTC production run both reported `imageIncluded=true`. At 07:00 UTC, all five image agents reported `imageIncluded=true`, responded to the broker, saved a distinct `ai_trading_log` row, and logged a permanent chart URL. The broker ledger showed all ten accounts receiving the same one-minute candle. The dashboard recorded one matched decision for each pair after its common 07:00 UTC start; all five first signals agreed. This is a functioning comparison, not yet evidence that either decision path performs better.

The existing numeric accounts have earlier positions and P&L; the new image accounts began flat at $50,000. Raw lifetime P&L is therefore not a fair pair comparison. The dashboard counts same-signal decisions on identical bars after 2026-09-24 07:00 UTC. Compare forward net P&L, drawdown, fill count, and decision latency from that start time, while accounting for different starting positions. Account context can still differ when one side takes a trade, so a later clean trial should start both sides of each pair from fresh, synchronized ledgers. Do not reset an account with an open position just to force alignment.
