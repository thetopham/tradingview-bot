...
@dashboard_bp.route("/market_state", methods=["GET"])
def market_state_view() -> Response:
    """Return the persisted market regime/state (best-effort).

    Protected by the same basic-auth middleware as the rest of the dashboard.
    """
        ...
        return jsonify(
            {
                ...
                "saved_at": None,
                "symbols": {},
                "note": "market state file not found",
                "path": str(state_path),
            }
        )
    except Exception as exc:
        return jsonify({"error": str(exc), "path": str(state_path)}), 500
