...
def _load_trade_state():
    ...
    with _state_lock:
        state_data = None
        for path in (STATE_PATH, STATE_BAK_PATH):
            if not path.exists():
                continue
            try:
                ...
                logging.info(
                    "Loaded trade state from %s", path
                )
                break
            except Exception as exc:
                logging.warning("Failed to load trade state from %s: %s", path, exc)
                state_data = None

        if state_data is None:
            logging.warning("Starting with empty trade state; no valid state files found")
            trade_meta = {}
            recent_closures = {}
            _last_save_ts = 0.0
            return

        loaded_meta = state_data.get("trade_meta", {}) or {}
        trade_meta = {}
        for raw_key, meta in loaded_meta.items():
            if isinstance(raw_key, str):
                normalized_key = raw_key
            elif isinstance(raw_key, (list, tuple)) and len(raw_key) == 2:
                normalized_key = _tm_key(raw_key[0], raw_key[1])
            else:
                continue
            trade_meta[normalized_key] = meta

        loaded_closures = state_data.get("recent_closures", {}) or {}
        recent_closures = {}
        for raw_key, ts in loaded_closures.items():
            if isinstance(raw_key, str):
                normalized_key = raw_key
            elif isinstance(raw_key, (list, tuple)) and len(raw_key) == 2:
                normalized_key = _tm_key(raw_key[0], raw_key[1])
                ...
                continue
            recent_closures[normalized_key] = ts

        _last_save_ts = time.time()
        logging.info(
            "Trade state loaded: %s sessions, %s recent closures",
            len(trade_meta),
            len(recent_closures),
        )
        ...
        try:
            ...
            if STATE_PATH.exists():
                try:
                    ...
                    logging.warning("Failed to create backup trade state: %s", exc)
            ...
            _last_save_ts = now
            logging.info(
                "Saved trade state with %s sessions and %s recent closures",
                len(trade_meta),
                len(recent_closures),
            )
        except Exception as exc:
...
def track_trade(
    ...
    trade_meta[_tm_key(acct_id, cid)] = meta
    _save_trade_state()
